...
const httpProxy = options => (req, res, next) => {
  if ('rawBody' in req) {
    // restream body
...
module.exports = {
  ...
  errorHandler: () => (err, req, res, next) => { // eslint-disable-line no-unused-vars
    if (err instanceof Error && (err.name === 'ValidationError' || err.name === 'SyntaxError')) {
      err.statusCode = 400; // eslint-disable-line no-param-reassign
      if (err.isJoi) {
    ...
    if (err instanceof Error && err.name === 'PayloadTooLargeError') {
      err.statusCode = 413; // eslint-disable-line no-param-reassign
    }
